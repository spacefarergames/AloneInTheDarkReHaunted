...
void osystem_startFrame()
{
    ...
    else
    {
        ...
        // Only reset to backbuffer if post-processing is NOT active
        // (post-processing beginScene() already bound the offscreen FB to view 0)
        bool ppActive = g_postProcessing && (g_remasterConfig.postProcessing.enableBloom || g_remasterConfig.postProcessing.enableFilmGrain || g_remasterConfig.postProcessing.enableSSAO);
        if (!ppActive)
        {
            bgfx::setViewFrameBuffer(gameViewId, BGFX_INVALID_HANDLE);
        }
    }
...
void osystem_flushPendingPrimitives()
{
    ...
    if (numUsedFlatVertices)
    {
        ...
        // If flushing shadow, test stencil==0 (only draw where no mask)
...
void osystem_createMask(const std::array<u8, 320 * 200>& mask, int roomId, int maskId, int maskX1, int maskY1, int maskX2, int maskY2)
{
    ...
    maskTextures[roomId][maskId].maskTexture = bgfx::createTexture2D(320, 200, false, 1, bgfx::TextureFormat::R8,
        BGFX_SAMPLER_U_CLAMP | BGFX_SAMPLER_V_CLAMP,
        bgfx::copy(mask.data(), 320 * 200));
