...
//
// Animation system, frame interpolation, and body animation
///////////////////////////////////////////////////////////////////////////////
...
int InitAnim(int animNum,int animType, int animInfo)
{
    if(animNum == currentProcessedActorPtr->ANIM)
    {
        if(!(currentProcessedActorPtr->objectType & AF_ANIMATED))
        {
            ...
            {
                sAnimation* pAnim = HQR_Get(HQ_Anims, animNum);
                sBody* pBody = HQR_Get(HQ_Bodys, currentProcessedActorPtr->bodyNum);
                if (pAnim && pBody)
                {
                    SetAnimObjet(currentProcessedActorPtr->frame, pAnim, pBody);
                }
            }
        ...
        if (newAnim == -2) // completely stop anim and add actor to background
        {
            ...
            if (!(currentProcessedActorPtr->newAnimType & ANIM_RESET)) // Reset
            {
                ...
                // TODO: AITD3 has some extra code here to handle bufferAnimCounter
                {
                    ...
                    if (pBody)
                    {
                        StockInterAnim(BufferAnim[bufferAnimCounter], pBody);
                    }
                }
            ...
            {
                sAnimation* pAnim = HQR_Get(HQ_Anims, newAnim);
                if (pAnim)
                {
                    currentProcessedActorPtr->numOfFrames = GetNbFramesAnim(pAnim);
                }
            }
        }
    }
