    ...
    for(int i =0;i<hqrPtr->numMaxEntry;i++)
    {
        if (hqrPtr->entries[i].ptr)
        {
            if constexpr (std::is_same_v<T, char>) {
                ...
                // Safe deletion for sBody - check if already deleted
                sBody* body = hqrPtr->entries[i].ptr;
                if (body && !body->m_isDeleted) {
                    body->m_isDeleted = true;
                    if (!SafeDelete::SafeDeleteObject(hqrPtr->entries[i].ptr)) {
                        // SafeDelete caught an exception, just null the pointer
                        hqrPtr->entries[i].ptr = nullptr;
                        SafeDelete::stats.doubleDeletesPrevented++;
                    }
                } else {
                    // Already marked as deleted, just null it
                    hqrPtr->entries[i].ptr = nullptr;
                    SafeDelete::stats.doubleDeletesPrevented++;
                }
            }
            else {
                // Use safe delete for other types
                SafeDelete::SafeDeleteObject(hqrPtr->entries[i].ptr);
                if (hqrPtr->entries[i].ptr) {
                    // If SafeDelete failed, just null it
                    hqrPtr->entries[i].ptr = nullptr;
                }
            }
        }
