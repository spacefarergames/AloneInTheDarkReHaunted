...
///////////////////////////////////////////////////////////////////////////////
// VOC Pre-Decode Cache for Book Reading
///////////////////////////////////////////////////////////////////////////////

struct PreDecodedVocPage
{
    unsigned char* wavData = nullptr;  // Pre-decoded WAV buffer
    size_t wavSize = 0;
    std::mutex mtx;
...
void osystem_playVocPageLines(int vocIndex, int page, int numLines)
{
    ...
    // CACHE CHECK: Try to get pre-decoded page from cache
    ...
    {
        // Use cached pre-decoded WAV
        printf(VO_OK "Playing VOC page from cache: book %d page %d (%zu bytes)" CON_RESET "\n",
               vocIndex, page, cachedWavSize);

        s_voStream = new SoLoud::WavStream();
        SoLoud::result res = s_voStream->loadMem(cachedWav, cachedWavSize, false, false);
        if (res == SoLoud::SO_NO_ERROR)
        {
            // NOTE: Don't store cachedWav in s_voBuf - it's managed by the cache
            s_voHandle = gSoloud->play(*s_voStream);
            if (g_voPitchMultiplier != 1.0f)
                gSoloud->setRelativePlaySpeed(s_voHandle, g_voPitchMultiplier);
            printf(VO_OK "Playing VOC page from cache: book %d page %d (instant playback)" CON_RESET "\n",
                   vocIndex, page);
            return;  // Cache hit - use cached data
        }
        else
        {
            delete s_voStream;
            s_voStream = nullptr;
            printf(VO_WARN "Failed to play cached VOC for book %d page %d" CON_RESET "\n", vocIndex, page);
            // Fall through to decode from scratch
        }
    }

    // CACHE MISS: Decode from scratch (existing logic)
...
// Pre-decode a VOC page in the background. Schedules a HIGH priority job
// to load and decode the VOC file, storing the result in the cache.
// This allows seamless page turns during book reading.
void osystem_preDecodeVocPage(int vocIndex, int page)
{
    // Don't pre-load if job system not initialized
    if (!JobSystem::isInitialized())
        return;

    uint32_t cacheKey = makeVocCacheKey(vocIndex, page);

    // Check if already in cache or being decoded
    {
    ...
    // Insert into cache (mark as in-progress)
    {
        std::lock_guard<std::mutex> lock(s_vocCacheMutex);
        s_vocPageCache[cacheKey] = cacheEntry;
    }

    // Schedule decoding job
    JobSystem::instance().scheduleJob(
        JobSystem::JobType::DECODE_VOC,
        JobSystem::Priority::HIGH,
        [vocIndex, page, cacheEntry, cacheKey]() {
            // Load VOC data from archive
            char vocFilename[32];
            snprintf(vocFilename, sizeof(vocFilename), "%02d%02d00.VOC", vocIndex, page);

            const HDArchiveEntry* entry = findAudioEntry(vocFilename);
            if (!entry)
            {
                printf(VO_WARN "Failed to find VOC page %d in archive" CON_RESET "\n", page);
                return;
            }

            size_t vocSize = 0;
            unsigned char* vocData = readAudioEntry(entry, &vocSize);
            if (!vocData)
            {
                printf(VO_WARN "Failed to load VOC page %d from archive" CON_RESET "\n", page);
                return;
            }
            ...
            size_t wavSize = 0;
            if (!vocDecodeToWav(vocData, vocSize, &wavData, &wavSize))
            {
                delete[] vocData;
                printf(VO_WARN "Failed to decode VOC page %d" CON_RESET "\n", page);
                return;
            }
            ...
            // Store in cache entry
            {
                ...
                cacheEntry->wavData = wavData;
                cacheEntry->wavSize = wavSize;
                cacheEntry->ready = true;
            }
            cacheEntry->cv.notify_all();

            delete[] vocData;
            printf(VO_OK "Pre-decoded VOC page %d (%zu bytes)" CON_RESET "\n", page, wavSize);
        });
}

// Try to get a pre-decoded VOC page. Returns the WAV data if ready, nullptr otherwise.
// Caller should not modify or delete the returned data - it's owned by the cache.
unsigned char* osystem_tryGetPreDecodedVocPage(int vocIndex, int page, size_t* outSize)
{
    ...
    std::lock_guard<std::mutex> entryLock(cacheEntry->mtx);

    if (!cacheEntry->ready)
        return nullptr;  // Still decoding

    if (outSize)
        *outSize = cacheEntry->wavSize;

    return cacheEntry->wavData;
}

// Clear the entire VOC cache. Called on shutdown or when memory pressure requires it.
