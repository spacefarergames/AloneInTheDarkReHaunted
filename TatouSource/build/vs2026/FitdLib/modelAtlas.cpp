...
// Load atlas PNG data from atlases.hda archive or fall back to filesystem
// Returns RGBA8 pixel data (caller must free with stbi_image_free)
// Sets w, h, channels on success; returns nullptr on failure
// NOTE: First checks filesystem to avoid archive API conflicts with other systems
static unsigned char* loadAtlasDataFromArchiveOrFile(const std::string& filePath, int& w, int& h, int& channels)
{
    // First, try to load from individual file on filesystem
    // This avoids conflicts with other archive operations (e.g., hd_backgrounds.hda)
    ...
    // If filesystem load fails, try archive as fallback
    // Extract just the filename for archive entry lookup
    size_t lastSlash = filePath.find_last_of("/\\");
    std::string fileName = (lastSlash != std::string::npos) ? filePath.substr(lastSlash + 1) : filePath;

    // Try to load from atlases.hda archive
    ...
    if (HDArchive::open(archivePath.c_str()))
    {
        ...
        if (entry)
        {
            ...
            {
                // Use stbi_load_from_memory to decode PNG from buffer
