...
void GereFrappe(void)
{
    ...
    case 5: // DO_TIR (Fire gun - ALWAYS hits with instant raycast)
        {
            ...
            // ALWAYS USE INSTANT RAYCAST - Guaranteed hit on line-of-sight target
            printf("  -> Using instant raycast for guaranteed hit\n");
            ...
            if (hitActorIdx >= 0)
            {
                ...
                printf("  -> Applying damage: hitForce=%d, actor life BEFORE=%d\n", 
                    currentProcessedActorPtr->hitForce, hitActorPtr->life);

                hitActorPtr->life -= currentProcessedActorPtr->hitForce;

                printf("  -> Actor life AFTER=%d\n", hitActorPtr->life);
            ...
            {
                printf("  -> Raycast MISS (hitActorIdx=%d) - no target in line of sight\n", hitActorIdx);
            }
